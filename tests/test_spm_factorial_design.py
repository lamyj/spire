...
class TestSPMFactorialDesign(unittest.TestCase):
    ...
    def test_masking(self):
        ...
        self.assertEqual(masking(1), expected)
        
        masking = spire.spm.factorial_design.Masking(
            threshold_mode="absolute", threshold=123)
        self.assertEqual(
            masking(1), expected.replace("tm.tm_none = 1", "tma.athresh = 123"))
        ...
        self.assertEqual(masking(1), expected.replace("im = 1", "im = 0"))
        
        masking = spire.spm.factorial_design.Masking(explicit="foo.nii,1")
        self.assertEqual(masking(1), 
    ...
    def test_global_calculation(self):
        ...
        self.assertEqual(global_calculation(1), expected)
        
        global_calculation = spire.spm.factorial_design.GlobalCalculation(
            "user", [12, 34])
        self.assertEqual(
            global_calculation(1), 
            expected.replace("g_omit = 1", "g_user.global_uval = [12;34]"))
        
        global_calculation = spire.spm.factorial_design.GlobalCalculation("mean")
        self.assertEqual(
            global_calculation(1), expected.replace("g_omit = 1", "g_mean = 1"))
    ...
    def test_global_normalization(self):
        ...
        self.assertEqual(global_normalization(1), expected)
    ...
    def test_covariates(self):
        ...
        expected = textwrap.dedent("""\
            ...
            """)
        self.assertEqual(covariates(1), expected)
    ...
    def test_two_samples_t_test(self):
        ...
        self.assertEqual(test(1), expected.replace("dept = 0", "dept = 1"))
        
        test = spire.spm.factorial_design.TwoSamplesTTest(
            *scans, equal_variance=True)
        self.assertEqual(test(1), expected.replace("variance = 1", "variance = 0"))
        
        test = spire.spm.factorial_design.TwoSamplesTTest(
            *scans, grand_mean_scaling=True)
        self.assertEqual(test(1), expected.replace("gmsca = 0", "gmsca = 1"))
        
        test = spire.spm.factorial_design.TwoSamplesTTest(*scans, ancova=True)
        self.assertEqual(test(1), expected.replace("ancova = 0", "ancova = 1"))
